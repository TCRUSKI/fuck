	...
	if addItems[entity:GetNetWorkId()] ~= nil then
		for k, v in pairs(addItems[entity:GetNetWorkId()]) do 
			...
			ah[k].Paint = function( self, w, h )
				...
				surface.DrawTexturedRect( 366, 14, 50, 25  )			
				draw.RoundedBox( 0, 15, 0, w, 5, v.Color )		
				draw.RoundedBox( 0, 20, 12, 55, 55, Color( 30, 30, 30 ) )
				draw.SimpleText( v.Name, "XanathF4_Font32", 95, 9, Color( 255, 255, 255 ), 0, 0 )
				draw.SimpleText( v.Description, "XanathF4_Font20", 95, 45, Color( 255, 255, 255 ), 0, 0 )		
				draw.SimpleText( "$"..v.Price, "XanathF4_Font20", 389, 27, Color( 255, 255, 255 ), 1, 1 )	
			end	
			ah[k].model = vgui.Create( "DModelPanel", ah[k] )
			ah[k].model:SetSize( 60, 60 )
			...
			ah[k].button:SetPos( 427, 14 )
			ah[k].button:SetSize( 101, 25 )
			ah[k].button:SetText( "" )
